...
namespace Math64x61:
    ...
    func pow_int{range_check_ptr}(x : felt, y : felt) -> (res : felt):
        alloc_locals
        let (exp_sign) = sign(y)
    ...
    func sqrt{range_check_ptr}(x : felt) -> (res : felt):
        alloc_locals
        let (root) = std_sqrt(x)
        let (scale_root) = std_sqrt(Math64x61_FRACT_PART)
    ...
    func msb{range_check_ptr}(x : felt) -> (res : felt):
        alloc_locals
    ...
    func exp2{range_check_ptr}(x : felt) -> (res : felt):
        ...
        let (int_res) = pow_int(2 * Math64x61_ONE, int_part)
        ...
        if exp_sign == -1:
            let (res_i) = div(Math64x61_ONE, res_u)
            assert64x61(res_i)
            return (res_i)
        else:
            assert64x61(res_u)
    ...
    func log2{range_check_ptr}(x : felt) -> (res : felt):
        ...
        if is_frac == 1:
            let (div) = div(Math64x61_ONE, x)
            let (res_i) = log2(div)
            return (-res_i)
        end

        let (x_over_two, _) = unsigned_div_rem(x, 2)
        let (b) = msb(x_over_two)
        let (divisor) = pow(2, b)
